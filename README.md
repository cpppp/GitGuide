...
- **源码级别分析（V3.2）** - 支持按文件路径精确检索，可分析特定文件如 "analyze main.py"
...
| 向量数据库   | ChromaDB            | 0.4+          | RAG 知识库存储                |
| Embeddings | HuggingFace         | all-MiniLM-L6-v2 | 免费高效的向量嵌入模型        |
...
│   ├── workflow.py           # 工作流 - 状态管理+并行调度+质量审核
│   ├── chat_agent.py         # 聊天 Agent（V3.2，原 chat.py 重命名）
│   ├── analyzers/            # 分析团队
│   │   ├── type_analyzer.py
...
│   ├── git_tools.py           # Git 仓库操作
│   └── file_tools.py          # 文件读取分析
│
├── core/                      # 核心模块
│   ├── config.py              # 配置管理
│   ├── sop.py                 # SOP 流程定义
...
2. **输入仓库 URL** - 在首页输入 GitHub 仓库 URL（例如：`https://github.com/666ghj/DeepSearchAgent-Demo`）
...
- **源码级别分析** - 支持按文件路径精确检索，如"分析 main.py"
- **RAG 知识库** - 基于 Chroma 向量数据库和 HuggingFace 嵌入模型
- **多轮对话** - ConversationManager 管理对话历史和上下文
- **源码索引** - SourceCodeIndexer 支持多语言源码索引构建
...
### v3.1 ✅ 已完成
- [x] 7 种文档类型（快速入门、项目概览、架构设计、安装部署、使用教程、开发指南、故障排查）
- [x] 5 种辅助素材（架构图、目录树、代码图谱、示例代码、学习路径）
- [x] 质量控制机制
- [x] 文档质量评分系统

### v3.2 ✅ 已完成
- [x] AI 问答 RAG 知识库（Chroma + HuggingFace Embeddings）
- [x] 源码级别分析（按文件路径精确检索）
- [x] 多轮对话上下文管理
- [x] 源码索引构建器（多语言支持）
