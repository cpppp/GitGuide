...
GitGuide 采用基于 Streamlit + LangChain 的多 Agent 架构，通过协调多个专业化 Agent 来完成 GitHub 仓库的分析、文档生成和智能问答功能。
...
为解决 Streamlit 同步执行导致的 UI 阻塞问题，引入了 Redis + RQ 异步任务队列：
...
| 组件 | 文件 | 说明 |
|------|------|------|
| 任务队列管理 | `backend/tasks.py` | 提交任务、查询状态、取消任务 |
| 后台执行器 | `backend/worker.py` | RQ Worker 执行的任务函数 |
