...
| POST            | `/api/chat`                    | AI 问答（V3.2支持源码分析） |
| GET             | `/api/chat/history`            | 获取问答历史 |
| DELETE          | `/api/chat/history`            | 清除问答历史 |
| POST            | `/api/chat/build-knowledge-base` | 构建知识库（V3.2新增） |
| GET             | `/api/chat/file-content`        | 获取文件内容（V3.2新增） |
...
```
用户问题 ──► ConversationManager ──► 文件路径提取 ──► RAGRetriever
                                                        │
                                                        ▼
                                                   混合检索
                                              （文件路径 + MMR + 关键词）
...
| 能力 | 说明 |
| :--- | :--- |
| **文件级别检索** | 用户可以问"分析 main.py"，AI 会精确检索并分析该文件 |
| **函数/类定位** | 自动识别用户询问的函数/类，返回完整定义 |
| **多轮上下文** | 连续追问同一文件时，保持上下文连贯 |
| **引用标注** | 回答中包含文件路径和行号引用 |
...
| V3.2 | AI问答源码分析       | 新增 SourceCodeIndexer、KnowledgeBuilder、RAGRetriever、ConversationManager，支持源码级别分析 |
